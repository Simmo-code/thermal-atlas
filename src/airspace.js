...
function pointInFeature(lon, lat, feature) {
  ...
  const polys = polygonSetsFromGeometry(feature.geometry);
...
function airspaceStyle(type) {
  ...
  if (t.includes("CTR")) {
    return { stroke: "#ff5a5a", fill: "rgba(255,90,90,0.12)" };
  }
  if (t.includes("TMA") || t === "A" || t.includes("CLASS A")) {
    return { stroke: "#ff9f43", fill: "rgba(255,159,67,0.10)" };
  }
  if (t === "D" || t.includes("CLASS D")) {
    return { stroke: "#4dabf7", fill: "rgba(77,171,247,0.10)" };
  }
  if (t === "C" || t.includes("CLASS C")) {
    return { stroke: "#a78bfa", fill: "rgba(167,139,250,0.10)" };
  }
  if (t.includes("TMZ")) {
    return { stroke: "#ffd43b", fill: "rgba(255,212,59,0.10)" };
  }
  if (t.includes("RMZ")) {
    return { stroke: "#63e6be", fill: "rgba(99,230,190,0.10)" };
  }
